...
function timeEnd(label: string, startTime: number) {
...
`;
	...
	try {
		...
		const apiCallStartTime = timeStart("Gemini API Call");
		const result = await model.generateContent(prompt);
		const response = await result.response;
		const text = response.text();
		timeEnd("Gemini API Call", apiCallStartTime);
...
export async function processChatMessageEnhanced(
	...
	const needsChunkedProcessing = isAnalyticalQuery(question, queryType) && records.length > 20;
	let aiResponse;
	let contextTime = 0;
	let aiTime = 0;
	let context = "";
	...
	if (needsChunkedProcessing) {
		console.log(`[CHAT PROCESSING] Detected analytical query with ${records.length} records - using chunked processing`);
		const chunkedStartTime = timeStart("Chunked Analytical Processing");
		aiResponse = await processChunkedAnalyticalQuery(question, records);
		aiTime = timeEnd("Chunked Analytical Processing", chunkedStartTime);
		...
		const contextStartTime = timeStart("Context Preparation");
		context = prepareContextForAI(records, queryForSearch);
		contextTime = timeEnd("Context Preparation", contextStartTime);
		...
		const aiStartTime = timeStart("AI Response Generation");
	...
	const sourceTime = timeEnd("Source Extraction", sourceStartTime);
	...
	console.log(`[TIMING-SUMMARY] - ${needsChunkedProcessing ? "Chunked " : ""}AI response generation: ${aiTime}ms`);
	...
	if (needsChunkedProcessing) {
		console.log(`[TIMING-SUMMARY] - Used chunked processing for analytical query`);
	}
