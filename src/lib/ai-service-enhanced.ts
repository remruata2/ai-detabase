 ...
 */
export function prepareContextForAI(records: SearchResult[], query?: string): string {
  const recordCount = records.length;
  console.log(`[CONTEXT-PREP] Processing ${recordCount} records for context generation`);
  if (records.length === 0) {
...
`;
  ...
  try {
    console.log(`[AI-GEN] Sending request to Gemini API, prompt size: ${prompt.length} characters`);
...
export async function processChatMessageEnhanced(
  ...
  console.log(`[CHAT PROCESSING] Starting context preparation for ${records.length} records`);
  const contextStartTime = timeStart('Context Preparation');
  const context = prepareContextForAI(records, queryForSearch);
  const contextTime = timeEnd('Context Preparation', contextStartTime);
  ...
  const aiTime = timeEnd('AI Response Generation', aiStartTime);
  ...
  console.log(`[TIMING-SUMMARY] - Total post-search time: ${contextTime + aiTime + sourceTime}ms`);
