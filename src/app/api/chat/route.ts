...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check usage limit
    const canChat = await checkLimit(parseInt(session.user.id), UsageType.chat_message);
    if (!canChat) {
      return NextResponse.json(
        { error: "Chat message limit exceeded. Please upgrade your plan." },
        { status: 429 }
      );
    }
    ...
    console.log(`[CHAT] User ${session.user.email} asked: "${message}"`);
    ...
    await trackUsage(parseInt(session.user.id), UsageType.chat_message);
    ...
    // Log the interaction
    console.log(
      `[CHAT] Response generated with ${result.sources.length} sources`
    );
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error("[CHAT] Error in GET endpoint:", error);
